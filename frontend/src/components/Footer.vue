<script setup></script>

<template>
  <footer class="-mt-3 rounded-t-2xl border-t-2 bg-white">
    <div class="mx-auto w-full max-w-screen-xl p-4 md:py-6">
      <div
        class="flex flex-col-reverse items-center gap-3 sm:items-center sm:justify-between md:flex-row"
      >
        <span
          class="block text-sm font-medium text-gray-500 dark:text-gray-400 sm:text-center"
          >© 2023 <span class="">ElitaWork</span>. All Rights Reserved.</span
        >
        <div class="flex flex-col items-end gap-2">
          <a
            class="flex items-center gap-2 text-gray-600 transition-all duration-300 hover:underline dark:text-gray-400"
            href="https://www.facebook.com/people/Elita-Work/pfbid06N8b2ygJWHi3EKDzxDU7s5eufqnQwhDe84QsCWAxMYPdaWcbgYh5cbtJBW5zwNFjl/?locale=hu_HU"
          >
            <span class="font-jost">Мы в Facebook:</span>
            <img
              alt="facebook"
              class="h-6 w-6"
              src="@/assets/images/facebook.svg"
            />
          </a>
          <a
            class="flex items-center gap-2 text-gray-600 transition-all duration-300 hover:underline dark:text-gray-400"
            href="https://invite.viber.com/?g2=AQAWwLRV%2BCTniFEXnomIiA3e6IG9cS6G%2B1GYxAKYMmGaOccpAtq9jNVN%2FGRsbetS"
          >
            <span class="font-jost">Мы в Viber:</span>
            <img
              alt="facebook"
              class="h-6 w-6"
              src="@/assets/images/viber.svg"
            />
          </a>
        </div>
      </div>
    </div>
  </footer>
</template>

<style scoped></style>
